...
\begin{abstract}
教室黑板与一体机屏幕在强光下易产生镜面反射，导致局部座位看不清板书。本文在单侧窗户平行光入射、黑板可绕底边小角度俯仰的设定下，先建立竖直截面内的反射模型，再在此基础上将座位区抽象为水平面上的长方形区域，用离散网格判定「反射光与视线是否共线（在阈值角内）」以刻画受害范围。对每个授课时刻，在最大俯仰角不超过 $\theta_{\max}$ 的硬约束下，用一维扫描求使前排代表座位消除眩光的\textbf{最小必要}俯仰角 $\theta^\ast(t)$。数值仿真表明：在示意参数下，早读后段（如 $8{:}00$）受害座位面积比例可由约 $11.6\%$ 降至约 $4.3\%$。据此提出可落地改造方案：黑板下沿加可调铰链、上沿设限位机构并配合分时段微调，可在不明显影响书写习惯的前提下降低眩光风险。
...
\end{equation}
该式本质上等价于「入射角等于反射角」：反射前后，光线在法向分量上符号翻转、切向分量保持不变，因此可得到上式。在本问题中，黑板/屏幕表面在强光下存在明显镜面分量，学生“看不清”主要发生在镜面高亮区域，故采用几何光学的镜面反射定律是合理的一阶近似。
...
为便于理解，4.1--4.4 的关系可概括为：4.1 给出基础物理定律（光线如何反射），4.2 在竖直截面中确定法向随转角变化的几何关系，4.3 将该反射关系推广到长方形教室座位网格并形成“受害判据”，4.4 在前述判据上定义逐时刻优化目标并求解最小必要角。即“物理定律 $\rightarrow$ 几何建模 $\rightarrow$ 受害判定 $\rightarrow$ 优化决策”的递进链条。
...
\begin{figure}[htbp]
  ...
  \label{fig:ratio}
\end{figure}
\noindent 图 \ref{fig:ratio} 直观显示了全天各时刻受影响比例的变化：早间时段两条曲线差异较大，说明旋转策略在该时段更有必要；临近中午后两条曲线逐步接近 0，表明自然入射方向变化后眩光风险显著减弱。
\FloatBarrier
